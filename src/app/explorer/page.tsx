...
import { ActionStatus, ActionType, ExplorerType, TransactionResponse } from '@/types';
import { formatAddress, formatAmount, formatTime, statusStyles, typeStyles } from '@/utils';
...
const ExplorerPage: React.FC = () => {
	...
	const { data: transactionHistory, isLoading, error } = useQuery({
        ...
        <div>
          <table className="w-full text-left text-sm text-tableTextPrimary">
            ...
            <tbody className="bg-tableRowBackground">
              {filteredTransactions && filteredTransactions.map((tx, index) => (
                <tr
                  key={tx.id}
                  className={`${
                    index % 2 === 0 ? 'bg-tableRowBackground' : 'bg-tableBackground'
                  } hover:bg-tableHover`}
                >
